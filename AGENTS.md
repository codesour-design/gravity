...
## Template slide / presentazioni — USARE SEMPRE QUESTO

> **Regola:** ogni volta che servono slide o una presentazione (Google Slides, deck, pitch,
> documento aziendale a slide), partire SEMPRE dal template ufficiale fatto a mano da Gloria.
> Non crearne uno nuovo da zero e non reinventare cover/colori/font.

- **File master (Google Slides):** `Gravity_Template`
  - URL: https://docs.google.com/presentation/d/1wcofnhmrZw9VWBZ1T5cZkRexA8x2Z_wqTS7cmqrf_2k/edit
  - Drive file ID: `1wcofnhmrZw9VWBZ1T5cZkRexA8x2Z_wqTS7cmqrf_2k`
  - Per una nuova presentazione: **duplicare questo file** (copy_file) e riempire le slide, oppure
    applicare i suoi layout. Mantenere cover, colori (`#3E00FB` / `#FF4A1C`), font (Oswald/Inter) e stile.
- **Layout disponibili (8):** Cover · Indice · Divisore di Sezione · Contenuto (titolo+testo+bullet) ·
  Due colonne · Card "3 pilastri" · Statistiche/numeri · Chiusura.
- **Generazione automatica / versione PPTX:** `presentation-template/` nel repo
  (`gravity_master.py` per i mastri, `gravity_deck.py` per impaginare da codice/JSON,
  `Gravity_Master.pptx`). Usare solo se serve un .pptx o build automatica; la fonte di verità
  visiva resta il file Google Slides di Gloria qui sopra.
...
> `main` è il branch stabile. Ogni designer lavora sul proprio branch e mergia solo quando il lavoro è pronto.
